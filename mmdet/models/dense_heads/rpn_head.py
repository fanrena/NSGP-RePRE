...
class RPNHead(AnchorHead):
    ...
    def predict_cus(self,
                x: Tuple[Tensor],
                batch_data_samples,
                rescale: bool = False,
                fixed_scores = None) -> InstanceList:
        ...
        predictions, ret_scores = self.predict_by_feat_cus(
            *outs, batch_img_metas=batch_img_metas, rescale=rescale, fixed_scores=fixed_scores, with_nms=False)
        return predictions, ret_scores

    
    def predict_by_feat_cus(self,
                        cls_scores: List[Tensor],
                        bbox_preds: List[Tensor],
        ...
        result_list = []
        scores_bool = fixed_scores == None
        fixed_scores = [None] * len(batch_img_metas) if fixed_scores == None else fixed_scores
        for img_id in range(len(batch_img_metas)):
            ...
            results, tmp_scores = self._predict_by_feat_single_cus(
                ...
                cfg=cfg,
                rescale=rescale,
                with_nms=with_nms, fixed_scores = fixed_scores[img_id])
            result_list.append(results)
            if scores_bool:
                fixed_scores[img_id] = tmp_scores
        return result_list, fixed_scores

    def _predict_by_feat_single_cus(self,
                                cls_score_list: List[Tensor],
                                bbox_pred_list: List[Tensor],
                                score_factor_list: List[Tensor],
                                mlvl_priors: List[Tensor],
                                ...
                                rescale: bool = False,
                                with_nms: bool = True,
                                fixed_scores=None) -> InstanceData:
        """Transform a single image's features extracted from the head into
        bbox results.
        ...
        for level_idx, (cls_score, bbox_pred, priors) in \
            ...
            if fixed_scores != None:
                scores = fixed_scores[level_idx]
        ...
        return self._bbox_post_process_cus(
            results=results, cfg=cfg, rescale=rescale, img_meta=img_meta), ret_scores
        
    
    def _bbox_post_process_cus(self,
                           results: InstanceData,
                           cfg: ConfigDict,
                           rescale: bool = False,
                           with_nms: bool = True,
    ...
    def keep_idxed(self, scores, score_thr, topk, results=None):
