...
2.  Install the package in editable mode along with its dependencies:
    ...
    pip install -v -e .
    ```
    *   `-v`: Enables verbose output for detailed installation logs.
    *   `-e`: Installs the project in editable mode, allowing code modifications to take effect without reinstalling.
